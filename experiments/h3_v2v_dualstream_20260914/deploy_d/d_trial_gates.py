"""New D-only gates; old A/B are frozen read-only safety/asset dependencies.

No old C proof or latest is accepted. No NPU work at import or verification.
The independently reviewed policy and D tiny verifier must exist before launch.
"""
...
D_VALIDATION_CODE = contract.EXPERIMENT / "validation_d"
D_VALIDATION_VERIFIER = D_VALIDATION_CODE / "d_validation_verifier.py"
...
def selected_profile(group, sample_id=DEFAULT_SAMPLE):
    ...
    if group != contract.GROUP or sample_id != contract.SAMPLE:
        raise RuntimeError("Only the fixed D eight-card color trial is allowed")
...
def checked_module(path, sha256, module_name):
    ...
    if record["sha256"] != sha256:
        raise RuntimeError("Reviewed D verifier dependency SHA changed")
    spec = importlib.util.spec_from_file_location(module_name, path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Cannot load the fixed reviewed D verifier")
    module = importlib.util.module_from_spec(spec)
    sys.modules[module_name] = module
    spec.loader.exec_module(module)
    if Path(module.__file__).resolve() != path:
        raise RuntimeError("D verifier imported from unexpected path")
    return module
...
def source_code_manifest(policy):
    ...
    for name, expected in policy["candidate_files"].items():
        ...
        if record["sha256"] != expected:
            raise RuntimeError("Approved D candidate differs: " + name)
        result["d_candidate/" + name] = record
    for name, expected in SUPPORT_SHA256.items():
        ...
        if record["sha256"] != policy["candidate_files"].get(relative, expected):
            raise RuntimeError("D actual shape/reference/packing support differs: " + name)
        result["d_model_support/" + name] = record
    for relative in ("vllm_omni/__init__.py", "vllm_omni/logger.py",
        ...
        result["d_vendor/" + relative] = file_record(VENDOR / relative)
    for name in ("d_trial_gates.py", "run_d_trial.py", "launch_d_trial.sh", "d_contract.py",
                 "d_execution_verifier.py", "reviewed_policy.json"):
        result["d_trial/" + name] = file_record(CODE / name)
    if (result["d_trial/d_execution_verifier.py"]["sha256"] != policy["execution_verifier_sha256"]
            or Path(contract.__file__).resolve() != CODE / "d_contract.py"):
        raise RuntimeError("Actual D policy/parser module does not match its reviewed dependency")
    tiny_record = file_record(D_VALIDATION_VERIFIER)
    if tiny_record["sha256"] != policy["tiny_verifier_sha256"]:
        raise RuntimeError("D own tiny verifier differs from approved policy")
    result["d_validation/d_validation_verifier.py"] = tiny_record
    pins_record = file_record(D_VALIDATION_CODE / "pins.json")
    if pins_record["sha256"] != policy["validation_pins_sha256"]:
        raise RuntimeError("D validation pins differ from approved policy")
    result["d_validation/pins.json"] = pins_record
    return result


def tiny_gate(group, host, policy):
    if group != contract.GROUP:
        raise RuntimeError("D requires its own complete SP8 tiny proof")
    contract.validate_policy(policy)
    tiny = policy["tiny_proof"]
    run = Path(tiny["run_directory"])
    profiles.canonical_private(run)
    for name, key in (("d_validation_status.json", "status_sha256"),
                      ("d_tiny.json", "result_sha256"),
                      ("source_manifest.json", "source_manifest_sha256")):
        if file_record(run/name)["sha256"] != tiny[key]:
            raise RuntimeError("D own tiny per-run proof file changed: " + name)
    status = json_file(run/"d_validation_status.json")
    expected = dict(case="D_tiny_SP8", mode=MODE, phase="completed", host=host,
                    ...
                    result_path=str(run/"d_tiny.json"), result_sha256=tiny["result_sha256"])
    if any(status.get(key) != value for key, value in expected.items()):
        raise RuntimeError("D needs a completed, cleaned-up own current-host tiny; C/B evidence forbidden")
    if json_file(run/"host_identity.json") != host:
        raise RuntimeError("D tiny host/boot changed")
    checker = checked_module(D_VALIDATION_VERIFIER, policy["tiny_verifier_sha256"],
                             "_reviewed_d_validation_verifier")
    ...
    if not isinstance(verified, dict) or verified.get("verified") is not True:
        raise RuntimeError("Reviewed D actual numeric/SP8 result verifier did not approve proof")
    return {"status_record": file_record(run/"d_validation_status.json"), "status": status,
            "result_record": file_record(run/"d_tiny.json"),
            "source_manifest_record": file_record(run/"source_manifest.json"), "verification": verified,
            "scope": "D own actual SP8 correctness and source/policy proof, never old C/B latest."}


def execution_gate(log_text, worker_pids, sample_id, *, run_id, policy, policy_sha256, requests=1):
    sample_profile(sample_id)
    checker = checked_module(contract.VERIFIER, policy["execution_verifier_sha256"],
                             "_reviewed_d_execution_verifier")
    return checker.parse_execution_records(log_text, worker_pids, run_id=run_id,
                                             policy=policy, policy_sha256=policy_sha256, requests=requests)


def logging_configuration():
    return {"version": 1, "disable_existing_loggers": False,
            "formatters": {"h3d": {"format": "H3D pid=%(process)d %(asctime)s %(levelname)s %(name)s %(message)s"}},
            "handlers": {"h3d": {"class": "logging.StreamHandler", "formatter": "h3d", "stream": "ext://sys.stderr"}},
            "loggers": {name: {"handlers": ["h3d"], "level": "INFO", "propagate": False}
...
def parse_full_load_evidence(log_text, weights):
    ...
    for marker, field in (('OPENVDN_B_LOAD_RECORD', 'load'), ('OPENVDN_B_CPU_LOADING_END', 'cpu_loading')):
        pattern = re.compile(r'H3D pid=(\d+) [^\n]*?' + marker + r' (\{[^\n]*\})')
