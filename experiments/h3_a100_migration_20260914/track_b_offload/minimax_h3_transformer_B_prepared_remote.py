...
from vllm_omni.diffusion.models.minimax_h3.openvdn_target_source_sparse import (
    source_target_sparse_softmax_attention,
)
...
class MiniMaxH3Attention(nn.Module):
    def __init__(
        ...
        *,
        ...
        self._ts_sparse_context: dict[str, Any] | None = None
    ...
    def _openvdn_softmax(
        ...
        sparse = self._ts_sparse_context
        if sparse is not None:
            return source_target_sparse_softmax_attention(
                q,
                k,
                v,
                sparse["source_layout"],
                layout,
                sparse["visible_source_frames"],
                self.softmax_scale,
...
class MiniMaxH3DiTModel(nn.Module):
    ...
    def __init__(
        ...
        route_file = os.environ.get("ZHONGHAO_H3_TS_ROUTE_FILE")
        self._ts_sparse_schedule = None
        if route_file:
            payload = json.loads(Path(route_file).read_text())
            if payload.get("schema") != "h3_b_nearest_grid_oracle_proxy_routes_v1":
                raise ValueError("Unsupported T-S sparse route schema")
            if int(payload.get("actual_layers", -1)) != len(self.blocks):
                raise ValueError("T-S sparse route layer count does not match DiT")
            self._ts_sparse_schedule = payload
        self._ts_sparse_step = 0
    ...
    def forward(self, **kwargs: Any) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.arch.openvdn_enabled:
            ...
            if self._ts_sparse_schedule is not None:
                openvdn_source_layout = infer_openvdn_layout(
                    source_video_pos, text_pos, img_position_ids, cu_seqlens,
                )
                if (
                    openvdn_source_layout.num_frames != openvdn_layout.num_frames
                    or openvdn_source_layout.tokens_per_frame != openvdn_layout.tokens_per_frame
                ):
                    raise ValueError("T-S sparse source and target grids differ")
        ...
        step_routes = None
        if self._ts_sparse_schedule is not None:
            step_key = str(self._ts_sparse_step)
            step_routes = self._ts_sparse_schedule["routes"].get(step_key)
            if step_routes is None:
                raise ValueError(f"Missing T-S sparse routes for denoising step {step_key}")
        for layer_index, block in enumerate(self.blocks):
            if step_routes is not None:
                visible = step_routes.get(str(layer_index))
                if visible is None:
                    raise ValueError(
                        f"Missing T-S sparse routes for step {self._ts_sparse_step}, "
                        f"layer {layer_index}"
                    )
                block.attn._ts_sparse_context = {
                    "source_layout": openvdn_source_layout,
                    "visible_source_frames": visible,
                }
            hidden = block(
                hidden,
                t_emb=t_emb,
            ...
            block.attn._ts_sparse_context = None
