...
[[ "${H3_PROFILE_VENDOR}" == "/cache/zhonghao/h3/profiling_d4_v1/candidates/${H3_PROFILE_CASE}/vllm-omni" ]] || exit 2
unset PYTHONPATH
source /cache/zhonghao/h3/env_h3_31731.sh
export PYTHONPATH="/cache/zhonghao/h3/profiling_d4_v1:${H3_PROFILE_VENDOR}${PYTHONPATH:+:${PYTHONPATH}}"
...
export TMPDIR="/cache/zhonghao/h3/tmp/p4_${H3_MINIMAL_RUN_ID}_${H3_PROFILE_CASE}"
[[ ! -e "${TMPDIR}" ]] || exit 2
mkdir "${TMPDIR}"
exec "${H3_ENV}/bin/vllm" serve "${H3_MODEL}/Ref2VA" \
  --omni --host 127.0.0.1 --port 19112 --trust-remote-code \
