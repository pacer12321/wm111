...
def configure_openvdn_from_env(od_config: Any, *, partition: str):
    ...
    if partition.lower() != "ref2va":
        raise ValueError("This OpenVDN transfer experiment requires the Ref2VA base")
    ...
    if not checkpoint_env:
        raise ValueError("Set ZHONGHAO_H3_OPENVDN_CHECKPOINT to the full DMD directory")
    checkpoint = Path(checkpoint_env).expanduser().resolve(strict=True)
    if not checkpoint.is_dir() or checkpoint.name != "stage-dmd-step-250":
        raise ValueError("This candidate requires the complete stage-dmd-step-250 artifact")
    for rel in ("linear_branch/model.safetensors", "adapters/default/adapter_model.safetensors",
                "adapters/turbo/adapter_model.safetensors", "metadata.json"):
    ...
    logger.info("OPENVDN_DMD8_CONFIG checkpoint=%s base_partition=%s TP=1", checkpoint, partition)
