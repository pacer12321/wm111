...
class MiniMaxH3Attention(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.openvdn_source_local_radius = int(
            os.environ.get("ZHONGHAO_H3_SS_TEMPORAL_RADIUS", "-1")
        )
    ...
    def _openvdn_softmax(
        ...
        if interleaved_map is not None:
            ...
            if self.openvdn_source_local_radius >= 0 and source_layout is None:
                raise ValueError("S-S temporal-local attention requires a source layout")
            return _interleaved_openvdn_softmax_attention(
                ...
                source_local_radius=self.openvdn_source_local_radius,
            )
        if self.openvdn_source_local_radius >= 0:
            raise ValueError("S-S temporal-local attention currently requires interleaved Ulysses")
...
class MiniMaxH3DiTModel(nn.Module):
    ...
    def forward(self, **kwargs: Any) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.arch.openvdn_enabled:
            ...
            if self._source_hybrid_enabled or self._ss_temporal_radius >= 0:
...
_INTERLEAVED_SS_LOCAL_PLAN_CACHE: dict[tuple, tuple] = {}


def _interleaved_ss_local_plan(
    target_layout: OpenVDNLayout,
    source_layout: OpenVDNLayout,
    device: torch.device,
    radius: int,
) -> tuple[torch.Tensor, tuple[tuple[torch.Tensor, torch.Tensor], ...]]:
    """Preserve the target OpenVDN plan while making only S-to-S temporal-local.

    Source queries keep every non-source key (therefore S-to-T remains dense),
    but source-video keys are restricted to the query frame's +/- radius
    latent-time slices.  The plan is static for a request shape and reused by
    every layer and denoising step.
    """
    if radius < 0:
        raise ValueError("source temporal-local radius must be non-negative")
    if source_layout.used_len != target_layout.used_len:
        raise ValueError("source and target layouts must cover the same request")
    if max(source_layout.video_start, target_layout.video_start) < min(
        source_layout.video_end, target_layout.video_end
    ):
        raise ValueError("source and target video segments must not overlap")
    cache_key = (target_layout, source_layout, int(radius), str(device))
    cached = _INTERLEAVED_SS_LOCAL_PLAN_CACHE.get(cache_key)
    if cached is not None:
        return cached

    dense_q, _all_k, target_groups = _softmax_plan(target_layout, device)
    source_start, source_end = source_layout.video_start, source_layout.video_end
    dense_q = dense_q[(dense_q < source_start) | (dense_q >= source_end)]
    non_source_keys = torch.cat(
        (
            torch.arange(0, source_start, device=device, dtype=torch.long),
            torch.arange(source_end, source_layout.used_len, device=device, dtype=torch.long),
        )
    )
    source_groups: list[tuple[torch.Tensor, torch.Tensor]] = []
    for frame in range(source_layout.num_frames):
        q_idx = _frame_indices(source_layout, frame, frame + 1, device)
        lo = max(0, frame - radius)
        hi = min(source_layout.num_frames, frame + radius + 1)
        local_source_keys = _frame_indices(source_layout, lo, hi, device)
        k_idx = torch.cat((non_source_keys, local_source_keys)).sort().values
        source_groups.append((q_idx, k_idx))
    cached = dense_q, tuple(target_groups) + tuple(source_groups)
    _INTERLEAVED_SS_LOCAL_PLAN_CACHE[cache_key] = cached
    return cached
...
def _interleaved_openvdn_softmax_attention(
    ...
    if source_local_radius >= 0:
        if source_layout is None:
            raise ValueError("source layout is required for S-S temporal-local attention")
        dense_q_logical, groups_logical = _interleaved_ss_local_plan(
            layout, source_layout, query.device, source_local_radius
        )
    else:
