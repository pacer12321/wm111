...
class MiniMaxH3Attention(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Read-only target-query Q/K/V capture used by the T-S/T-T frame-map
        # probe.  It is deliberately independent from the attention route so
        ...
        *,
        ...
        if capture is not None:
            ...
            if (
                ...
                logger.info(
                    "FASTEST_TS_TARGET_QKV_CAPTURED step=%d layer=%d rank=%d "
...
class MiniMaxH3DiTModel(nn.Module):
    ...
    def forward(self, **kwargs: Any) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if ss_capture_dir and self._spotedit_step in ss_capture_steps:
            ...
            target_positions_all = img_pos.to(device).index_select(0, target_visual)
            source_positions = source_positions[source_positions < used_len]
            target_valid = target_positions_all < used_len
            target_positions = target_positions_all[target_valid]
            ...
            if spot_payload is None or spot_refresh:
                eligible = torch.ones(
                    target_positions.numel(), device=device, dtype=torch.bool
                )
                query_policy = "all_target_queries_refresh"
            else:
                eligible = spot_payload["active_target_mask"].to(
                    device=device, dtype=torch.bool
                )[target_valid]
                query_policy = "active_target_queries_skip"
            eligible_grid = eligible.view(frames, height, width)
            sampled_parts = []
            for frame in range(frames):
                frame_positions = target_grid[frame].reshape(-1)
                candidates = frame_positions[eligible_grid[frame].reshape(-1)]
                if candidates.numel() == 0:
                    continue
                sample_count = min(16, int(candidates.numel()))
                offsets = torch.linspace(
                    0, candidates.numel() - 1, steps=sample_count, device=device
                ).round().long().unique()
                sampled_parts.append(candidates.index_select(0, offsets))
            if not sampled_parts:
                raise ValueError("T-S capture found no actually executed target queries")
            ...
            ss_capture_context = {
                ...
                "spotedit_refresh": bool(spot_refresh),
