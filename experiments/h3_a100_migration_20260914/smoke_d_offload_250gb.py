...
from d_memory_guard import GIB, MAX_USAGE_BYTES, MemoryGuard
from retry_d_after_ffprobe import validate_repair

q = d.q
CONTROL = d.ROOT / 'd_offload_250gb_smoke_20260915'
HISTORY = q.WORK / 'history/30674_d_serial_old_guard_20260915'
...
def launch():
    ...
    assert old['status'] == 'failed' and old['pid'] == 1481881
    assert old['memory_guard_reason'].startswith('proactive memory stop:')
    assert q.pid_identity(1481881) is None
...
def run():
    ...
    q.STATE.update(pid=os.getpid(), case_order=['D'], gpu_uuids=d.UUIDS,
                   ...
                   memory_guard=True, usage_threshold_bytes=MAX_USAGE_BYTES, weight_loading='serial',
