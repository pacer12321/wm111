#!/usr/bin/env python3
"""31731 fixed USP8 B: frozen lake (default) or reverse, smoke/formal/cleanup.

Opt-in only. Imports independent reviewed validation safety helpers, never
mutates their globals. Original Stage-B B only: no C, training, retries or choices
...
class BTrialSupervisor(base.ProcessSupervisor):
    def __init__(self, group, sample_id=gates.DEFAULT_SAMPLE):
        ...
        self._request_order = []
        self.status = {'case': 'B', 'sample_id': self.sample.sample_id, 'group': group,
                       ...
                       'attention_policy': 'B original linear + local softmax; source globally visible; no C corresponding-frame restriction',
    ...
    def update(self, phase, **values):
        ...
        if self.run_dir is not None:
            ...
            base.atomic_json(self.selected.output_root / 'b_status.json', self.status)
    ...
    def acquire(self):
        ...
        new_run = self.selected.output_root / 'runs' / f'b_{stamp}_{self.run_id}'
        profiles.canonical_private(new_run)
        new_run.mkdir(parents=True, exist_ok=False)
        self.run_dir = new_run
        for path in (self.run_dir / 'output', gates.ROOT / 'tmp' / f'b_{self.run_id}'):
            profiles.canonical_private(path)
    ...
    def preflight(self):
        ...
        self.update('preflight_passed', resource_snapshot=idle, card_health_memory=health, host_memory=memory,
                    ...
                    note='B original linear branch + globally visible source; fresh smoke/load/PID gates before one formal request.')

    def env(self):
        env = os.environ.copy()
        # Never inherit an arbitrary candidate/checkpoint, C activation or Python
        ...
        env.update(H3_ROOT=str(self.run_dir), H3_OUTPUT=str(self.run_dir / 'output'), H3_MINIMAL_RUN_ID=self.run_id,
                   ...
                   H3_PORT=str(self.selected.master_port), H3_GROUP_ID=f'zhonghao_31731_B_{self.selected.group}_{self.run_id}',
    ...
    def launch(self):
        ...
        self.server = self.spawn(['bash', str(self.selected.code_root / 'launch_b_trial.sh')],
        ...
        while time.monotonic() < deadline:
            if self.server.poll() is not None:
                raise RuntimeError(f'B server exited during startup: {self.server.returncode}')
            try:
                ...
                pass
            time.sleep(2)
        raise TimeoutError('B server did not become healthy within 2700 seconds')

    def request(self, name, steps):
        expected = [('smoke_2step', 2), ('b_50step', 50)]
        ...
        if steps == 50 and not self.status.get('formal_smoke_gate_passed'):
            raise RuntimeError('B formal request cannot bypass the fresh same-service smoke gate')
        ...
        while child.poll() is None:
            if self.server.poll() is not None:
                raise RuntimeError('B server exited during request')
    ...
    def smoke_gate(self):
        ...
        if (not set(workers.values()).issubset(owned)
            ...
            raise RuntimeError('Selected-card workers are not all owned by this same B service')
    ...
    def run_requests(self):
        ...
        result = self.request('b_50step', 50)
...
def main(argv=None):
    ...
    owner = BTrialSupervisor(args.group, args.sample)
    ...
    try:
        ...
        try:
            ...
            if not clean and error is None:
                error = 'Owned B processes remain'
            ...
            owner.update('needs_attention' if owner.status.get('needs_attention') else ('failed' if error else 'formal_completed_review_required'),
                         error=error, finished_at=base.utc_now(), note='Stage-B original linear branch + global source only; no C. Edit quality, NFE and acceleration remain unreviewed.')
        finally:
            owner.release_locks()
