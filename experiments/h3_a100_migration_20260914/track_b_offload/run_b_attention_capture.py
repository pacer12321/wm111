"""Run one isolated Ref2VA+B/VDN request and capture pre-attention Q/K/V."""
...
CANDIDATE = ROOT / "attention_oracle_b_multilayer_code_20260916_v1/candidate_B_capture"
CAPTURE_DIR = EXPERIMENT / "qk_capture"
CAPTURE_STEPS = "0,12,24,36,48"
CAPTURE_LAYERS = "8,24,41"
TRANSFORMER_SHA256 = os.environ["ZHONGHAO_H3_B_CAPTURE_TRANSFORMER_SHA256"]
PIPELINE_SHA256 = os.environ["ZHONGHAO_H3_B_CAPTURE_PIPELINE_SHA256"]
...
STATE = {
    "experiment": "B Ref2VA+VDN; T-T local+linear, T-S dense teacher capture",
    "case": "B",
    "capture_steps": [0, 12, 24, 36, 48],
    "capture_layers": [8, 24, 41],
...
def main() -> None:
    ...
    for filename, expected in manifest["cases"]["B"]["model_files"].items():
        ...
        if filename == "pipeline_minimax_h3.py":
            source_expected = q.sha(source_candidate / q.REL / filename)
        if q.sha(source_candidate / q.REL / filename) != source_expected:
            raise RuntimeError(f"Prepared B candidate changed: {filename}")
        candidate_expected = source_expected
        if filename == "pipeline_minimax_h3.py":
        ...
        if q.sha(CANDIDATE / q.REL / filename) != candidate_expected:
            raise RuntimeError(f"Isolated B capture candidate changed: {filename}")
    ...
    environment["TMPDIR"] = str(ROOT / "tmp/b_vdn_attention_capture")
    ...
    (EXPERIMENT / "launch.json").write_text(json.dumps({
        ...
        "capture_steps": [0, 12, 24, 36, 48],
        "capture_layers": [8, 24, 41],
        "attention": "B: VDN c5/r1+linear T-T; dense T-S; B S-S unchanged",
        "clean_ablation_target": "only T-S visibility will change in Oracle analysis",
    }, indent=2))
        ...
        with (EXPERIMENT / "server.log").open("x") as log:
            ...
            while time.monotonic() < deadline:
                ...
                if server.poll() is not None:
                    raise RuntimeError("B/VDN model load failed; see server.log")
                try:
                ...
                raise TimeoutError("B/VDN model readiness timeout")
            update("model_ready_running_request")
            result = q.request(server, owned, "B", 50, EXPERIMENT / "formal_50step", manifest)
            result.update(
                attention="B VDN T-T local+linear; dense T-S",
                capture_steps=[0, 12, 24, 36, 48],
                capture_layers=[8, 24, 41],
