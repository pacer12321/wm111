#!/usr/bin/env python3
"""31731 fixed USP8 D: fixed shirt_red_couple_124, smoke/formal/cleanup.

Opt-in only. Imports independent reviewed validation safety helpers, never
mutates their globals. Only source self-attention changes; both target paths preserve old C. No training, retries or choices
...
class DTrialSupervisor(base.ProcessSupervisor):
    def __init__(self, group, sample_id=gates.DEFAULT_SAMPLE, *, policy_sha256):
        self.policy_sha256 = policy_sha256
        self.policy = gates.policy_gate(policy_sha256)
        ...
        self._request_order = []
        self.status = {'case': 'D', 'sample_id': self.sample.sample_id, 'group': group,
                       ...
                       'stage_b_checkpoint': str(gates.CHECKPOINT),
                       'attention_policy': self.policy['value']['attention'], 'mode': gates.MODE, 'training_performed': False,
                       'policy_sha256': self.policy_sha256,
                       'timeout_policy': '3600 seconds is only a waiting ceiling, never a speedup measurement'}

    def update(self, phase, **values):
        self.status.update(values, phase=phase, updated_at=base.utc_now())
        if self.run_dir is not None:
            base.atomic_json(self.run_dir / 'd_status.json', self.status)
            base.atomic_json(self.selected.output_root / 'd_status.json', self.status)
    ...
    def acquire(self):
        ...
        self.submission = gates.contract.claim_submission(
            self.selected.output_root, self.run_id, self.host, self.policy_sha256,
            self.status['supervisor_proc_identity'], base.utc_now())
        self.status['submission'] = self.submission
        stamp = time.strftime('%Y%m%dT%H%M%SZ', time.gmtime())
        new_run = self.selected.output_root / 'runs' / f'd_{stamp}_{self.run_id}'
        profiles.canonical_private(new_run)
        new_run.mkdir(parents=True, exist_ok=False)
        self.run_dir = new_run
        for path in (self.run_dir / 'output', gates.ROOT / 'tmp' / f'dmodel_{self.run_id}'):
            profiles.canonical_private(path)
    ...
    def evidence_snapshot(self):
        ...
        policy = gates.policy_gate(self.policy_sha256)
        if policy != self.policy:
            raise RuntimeError('Approved D policy changed after invocation')
        submission = gates.contract.verify_submission(self.submission, self.selected.output_root,
                                                        self.run_id, self.host, self.policy_sha256)
        sources = gates.source_code_manifest(policy['value'])
        transfer = gates.transfer_gate(self.host)
        runtime = gates.runtime_gate(self.host, sources)
        tiny = gates.tiny_gate(self.selected.group, self.host, policy['value'])
        ...
        return {'host': self.host, 'run_id': self.run_id, 'cards': list(self.selected.cards),
                ...
                'case': gates.CASE, 'mode': gates.MODE, 'policy': policy, 'submission': submission,
                'weights': gates.weight_manifest(transfer['verified']),
                'logging_config': gates.file_record(self.run_dir / 'formal_logging.json'),
                'timeout_policy': {'service_sync': REQUEST_TIMEOUT, 'client': REQUEST_TIMEOUT,
                                   'meaning': 'Waiting upper limits only; full HTTP request elapsed time is measured independently'},
                'precision_policy': {'cli_override': None, 'dit': 'Original BF16 Ref2VA + released Stage-B BF16 branch and LoRA merge; no precision override',
    ...
    def preflight(self):
        ...
        for executable in ('npu-smi', 'bash', 'curl', 'sha256sum', 'realpath'):
            if shutil.which(executable) is None:
                raise RuntimeError(f'Missing required executable: {executable}')
        base.atomic_json(self.run_dir / 'formal_logging.json', gates.logging_configuration())
        ...
        self.update('preflight_passed', resource_snapshot=idle, card_health_memory=health, host_memory=memory,
                    ...
                    note='D source-hybrid only; frozen reviewed policy and own SP8 proof plus fresh load/PID/execution gates.')

    def env(self):
        env = os.environ.copy()
        # Never inherit an arbitrary candidate/checkpoint or Python
        # override. The launcher establishes the approved private environment.
        for key in list(env):
            if (key.startswith(('ZHONGHAO_H3_OPENVDN', 'S0_', 'D_REVIEWED_POLICY', 'ZHONGHAO_H3_DUALSTREAM'))
                    or key in ('PYTHONPATH', 'VLLM_LOGGING_CONFIG_PATH')):
                env.pop(key)
        env.update(H3_ROOT=str(self.run_dir), H3_OUTPUT=str(self.run_dir / 'output'), H3_MINIMAL_RUN_ID=self.run_id,
                   ...
                   H3_TRIAL_SUPERVISOR_PID=str(os.getpid()), H3_TRIAL_CASE='D',
                   D_REVIEWED_POLICY_PATH=str(gates.contract.POLICY), D_REVIEWED_POLICY_SHA256=self.policy_sha256,
                   H3_PORT=str(self.selected.master_port), H3_GROUP_ID=f'zhonghao_31731_D_{self.selected.group}_{self.run_id}',
                   ASCEND_RT_VISIBLE_DEVICES=','.join(map(str, self.selected.cards)),
                   PYTHONNOUSERSITE='1', PYTHONDONTWRITEBYTECODE='1', ZHONGHAO_H3_OPENVDN='1',
                   ZHONGHAO_H3_OPENVDN_CHECKPOINT=str(gates.CHECKPOINT),
                   VLLM_CONFIGURE_LOGGING='1', VLLM_LOGGING_LEVEL='INFO', VLLM_LOGGING_COLOR='0',
                   VLLM_LOGGING_CONFIG_PATH=str(self.run_dir / 'formal_logging.json'),
                   VLLM_OMNI_VIDEO_SYNC_TIMEOUT=str(REQUEST_TIMEOUT))
        return env
    ...
    def launch(self):
        ...
        self.server = self.spawn(['bash', str(self.selected.code_root / 'launch_d_trial.sh')],
        ...
        while time.monotonic() < deadline:
            if self.server.poll() is not None:
                raise RuntimeError(f'D server exited during startup: {self.server.returncode}')
            try:
                ...
                pass
            time.sleep(2)
        raise TimeoutError('D server did not become healthy within 2700 seconds')

    def request(self, name, steps):
        expected = [('smoke_2step', 2), ('d_50step', 50)]
        ...
        if steps == 50 and not self.status.get('formal_smoke_gate_passed'):
            raise RuntimeError('D formal request cannot bypass the fresh same-service smoke gate')
        ...
        record = {'case': gates.CASE, 'mode': gates.MODE, 'policy_sha256': self.policy_sha256,
                  'started_at': base.utc_now(), 'host': self.host, 'group': self.selected.group, 'run_id': self.run_id,
        ...
        while child.poll() is None:
            if self.server.poll() is not None:
                raise RuntimeError('D server exited during request')
            if time.monotonic() - start > REQUEST_TIMEOUT + 20:
                raise TimeoutError('Request exceeded the 3600-second guard plus cleanup allowance')
            time.sleep(1)
    ...
    def smoke_gate(self):
        ...
        if (request.get('case') != gates.CASE or request.get('mode') != gates.MODE
                or request.get('policy_sha256') != self.policy_sha256
                or smoke != self.status.get('smoke_2step') or smoke.get('success') is not True
        ...
        if (not set(workers.values()).issubset(owned)
            ...
            raise RuntimeError('Selected-card workers are not all owned by this same D service')
        log_raw = (self.run_dir / 'server.log').read_bytes()
        loads = gates.parse_full_load_evidence(log_raw.decode('utf-8', errors='replace'), self.frozen['weights'])
        if set(loads) != set(workers.values()):
            raise RuntimeError('Eight complete load records must match exactly the eight live owned selected-card workers')
        execution = gates.execution_gate(log_raw.decode('utf-8', errors='replace'), set(workers.values()), self.sample.sample_id,
                                         run_id=self.run_id, policy=self.frozen['policy']['value'], policy_sha256=self.policy_sha256)
        if execution['rank_by_pid'] != {pid: card for card, pid in workers.items()}:
            raise RuntimeError('D execution ranks/devices differ from actual physical-card worker identities')
        proof = {'status': 'passed', 'case': gates.CASE, 'mode': gates.MODE,
                 'policy_sha256': self.policy_sha256, 'host': self.host, 'run_id': self.run_id, 'group': self.selected.group,
                 ...
                 'sample_id': self.sample.sample_id, 'loaded_records_by_pid': loads,
                 'execution_evidence': execution,
                 'server_log_prefix_bytes': len(log_raw),
                 'server_log_prefix_sha256': hashlib.sha256(log_raw).hexdigest(),
    ...
    def run_requests(self):
        ...
        result = self.request('d_50step', 50)
        ...
        self.assert_unchanged()
        raw = (self.run_dir / 'server.log').read_bytes()
        smoke_proof = self.status['formal_smoke_gate']
        prefix_size = smoke_proof['server_log_prefix_bytes']
        if hashlib.sha256(raw[:prefix_size]).hexdigest() != smoke_proof['server_log_prefix_sha256']:
            raise RuntimeError('D server log prefix changed after the smoke gate')
        workers = {int(pid) for pid in smoke_proof['loaded_records_by_pid']}
        execution = gates.execution_gate(raw.decode('utf-8', errors='replace'), workers, self.sample.sample_id,
                                         run_id=self.run_id, policy=self.frozen['policy']['value'], policy_sha256=self.policy_sha256, requests=2)
        base.atomic_json(self.run_dir / 'formal_execution_evidence.json', execution)
...
def main(argv=None):
    ...
    parser.add_argument('--policy-sha256', required=True, help='Explicit SHA256 of the reviewed D policy; no default')
    args = parser.parse_args(argv)
    if not args.allow_npu:
        parser.error('Nothing starts without explicit --allow-npu')
    owner = DTrialSupervisor(args.group, args.sample, policy_sha256=args.policy_sha256)
    ...
    try:
        ...
        try:
            ...
            if not clean and error is None:
                error = 'Owned D processes remain'
            ...
            owner.update('needs_attention' if owner.status.get('needs_attention') else ('failed' if error else 'formal_completed_review_required'),
                         error=error, finished_at=base.utc_now(), note='D source self-attention hybrid; target paths preserve C; no training. Edit quality, NFE and acceleration remain unreviewed.')
        finally:
            owner.release_locks()
