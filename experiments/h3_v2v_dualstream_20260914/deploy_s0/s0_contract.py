"""S0-only immutable admission and one-submission ledger; standard library only.
...
def validate_policy(row):
    if not isinstance(row, dict) or set(row) != POLICY_KEYS:
        raise RuntimeError("S0 policy schema incomplete or unknown fields")
    expected = dict(schema_version=1, case=CASE, mode=MODE, review_status="approved_for_inference",
                    ...
                    sample_id=SAMPLE, group=GROUP, candidate_vendor=str(VENDOR))
    if (type(row["schema_version"]) is not int
        ...
        raise RuntimeError("S0 policy is not explicitly finalized/reviewed training-free S0")
    ...
    if not valid_sha(row["execution_verifier_sha256"]):
        raise RuntimeError("Missing reviewed S0 execution verifier SHA")
    tiny = row["tiny_proof"]
    if (not isinstance(tiny, dict) or set(tiny) != TINY_KEYS or tiny.get("case") != "S0_tiny_SP8"
        ...
        raise RuntimeError("S0 needs an explicit own SP8 tiny proof, never old C/B latest")
    directory = Path(tiny["run_directory"])
    if (directory.parent != EXPERIMENT / "validation/runs"
            or re.fullmatch("[0-9]{8}T[0-9]{6}Z_" + tiny["run_id"], directory.name) is None):
        raise RuntimeError("Noncanonical fixed S0 tiny run directory")
    ...
    if (set(attention) != expected_attention_keys
            or attention["endpoint_policy"] not in ("vdn_anchors", "strict_local")
            or attention["auxiliary_policy"] not in ("preserve_existing", "source_closed")
        ...
        raise RuntimeError("S0 attention decisions or own tiny verifier are not explicitly approved")
    return row
...
def policy_gate(expected_sha256):
    ...
    if record["sha256"] != expected_sha256:
        raise RuntimeError("S0 policy file differs from explicit invocation SHA")
...
def claim_submission(output_root, run_id, host, policy_sha256, supervisor_identity, created_at):
    ...
    if (output_root != expected or output_root.resolve(strict=True) != output_root
        ...
        raise RuntimeError("Invalid fixed S0 submission identity")
    row = dict(schema_version=1, case=CASE, mode=MODE, group=GROUP, sample_id=SAMPLE,
               ...
               allowed_requests=[{"name": "smoke_2step", "steps": 2}, {"name": "s0_50step", "steps": 50}],
...
def verify_submission(claim, output_root, run_id, host, policy_sha256):
    ...
    if (claim != {"record": record, "value": row} or row.get("run_id") != run_id
        ...
        raise RuntimeError("S0 immutable submission claim changed or belongs to another run")
    return claim
