...
from .strict_source_attention import infer_strict_source_layout
from .strict_source_layout import validate_arch
from .dual_stream_adapter import (
    begin_forward as begin_d_forward,
    source_linear_layout,
    source_hybrid_softmax_attention,
    record_execution as record_d_execution,
)
...
class MiniMaxH3Attention(nn.Module):
    ...
    def _openvdn_softmax(
        ...
        if self.openvdn_interior_group_size:
            raise ValueError("D preserves C target c5/r1 and adds the same VDN structure to source")
        return source_hybrid_softmax_attention(
    ...
    def _run_openvdn_ulysses(
        ...
        softmax_head = self._openvdn_softmax(q_head[0], k_head[0], v_head[0], layout)
        ...
        # D: original target linear above is unchanged. Invoke the same module
        # separately on S, with its own interior statistics/conv/scan state.
        source_layout = source_linear_layout(layout)
        source_sums, source_counts = local_frame_sums_counts(x, source_layout, rank * x.shape[0])
        ...
        if not bool(torch.all(source_counts == source_layout.tokens_per_frame).item()):
            raise ValueError("D sequence shards do not cover every source interior frame exactly once")
        source_readout_head = self.linear_attention.forward_head_shard(
            (raw_q[0], raw_k[0], v_head[0]), beta_head[0, :, :, 0],
            source_sums / source_counts[:, None], source_layout,
            head_start=rank * (self.num_heads // world),
        )
        ...
        record_d_execution(layer_index=self.d_layer_index, rank=rank, world=world,
                           layout=layout, tensor=readout_local, heads=self.num_heads // world)
        ...
        *,
        ...
        if self.openvdn_enabled:
            ...
            if strategy.name == "ulysses":
                out, linear_readout_local = self._run_openvdn_ulysses(
                    x, q, k, v, qkv_raw, openvdn_layout, cu_seqlens, strategy,
                )
                ...
                out = self._openvdn_softmax(q, k, v, openvdn_layout)
            else:
        ...
        if self.openvdn_enabled and self.openvdn_linear_enabled:
            if linear_readout_local is not None:
                # Nonvisual/anchor/padding rows were zero-filled before the
                ...
                source_layout = source_linear_layout(openvdn_layout)
                source_readout = self.linear_attention(x, qkv_raw, source_layout)
                source_out = self.to_out_linear(source_readout.to(x.dtype))
                out[source_layout.video_start:source_layout.video_end] += source_out
                record_d_execution(layer_index=self.d_layer_index, rank=0, world=1,
                                   layout=openvdn_layout, tensor=out, heads=self.num_heads)
        return out
...
class MiniMaxH3DiTModel(nn.Module):
    ...
    def __init__(
        ...
        for d_layer_index, d_block in enumerate(self.blocks):
            d_block.attn.d_layer_index = d_layer_index
    ...
    def forward(self, **kwargs: Any) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        device = x.device
        # C metadata rejection precedes refiner and sequence collectives.
        ...
        # C layout was already validated before embedding.
        begin_d_forward()
