...
class MiniMaxH3Attention(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        total = x.shape[0]
        full_skip = spotedit_active_mask is not None and not spotedit_refresh
        active_indices = None
        stable_indices = None
        if full_skip:
            ...
            with _profile_scope("qkv_projection"):
                qkv_active, _ = self.qkv_proj(x.index_select(0, active_indices))
            q_active, k_active, v_active = qkv_active.split([q_size, kv_size, kv_size], dim=-1)
            q_active = q_active.view(-1, self.num_heads, self.head_dim)
            k_active = k_active.view(-1, self.num_kv_heads, self.head_dim)
            v_active = v_active.view(-1, self.num_kv_heads, self.head_dim)
            raw_q_active, raw_k_active = q_active, k_active
            with _profile_scope("qk_norm"):
                q_active = self.q_norm(q_active)
                k_active = self.k_norm(k_active)
            if rope_freqs is not None:
                with _profile_scope("rope"):
                    active_rope = rope_freqs.index_select(0, active_indices)
                    q_active = _apply_rope(q_active, active_rope)
                    k_active = _apply_rope(k_active, active_rope)

            cached_k = getattr(self, "_spotedit_stable_k", None)
            cached_v = getattr(self, "_spotedit_stable_v", None)
            cached_raw_k = getattr(self, "_spotedit_stable_raw_k", None)
            expected = (stable_indices.numel(), self.num_kv_heads, self.head_dim)
            if (
                cached_k is None
                or cached_v is None
                or cached_raw_k is None
                or tuple(cached_k.shape) != expected
                or tuple(cached_v.shape) != expected
                or tuple(cached_raw_k.shape) != expected
            ):
                raise RuntimeError("SpotEdit stable K/V cache is missing or has the wrong shape")

            q = torch.zeros(
                (total, self.num_heads, self.head_dim), device=x.device, dtype=q_active.dtype
            )
            k = torch.zeros(
                (total, self.num_kv_heads, self.head_dim), device=x.device, dtype=k_active.dtype
            )
            v = torch.zeros(
                (total, self.num_kv_heads, self.head_dim), device=x.device, dtype=v_active.dtype
            )
            raw_q = torch.zeros_like(q)
            raw_k = torch.zeros_like(k)
            q.index_copy_(0, active_indices, q_active)
            k.index_copy_(0, active_indices, k_active)
            v.index_copy_(0, active_indices, v_active)
            raw_q.index_copy_(0, active_indices, raw_q_active)
            raw_k.index_copy_(0, active_indices, raw_k_active)
            k.index_copy_(0, stable_indices, cached_k.to(device=x.device, dtype=k.dtype))
            v.index_copy_(0, stable_indices, cached_v.to(device=x.device, dtype=v.dtype))
            raw_k.index_copy_(
                0, stable_indices, cached_raw_k.to(device=x.device, dtype=raw_k.dtype)
            )
            qkv_raw = (raw_q, raw_k, v)
        else:
            with _profile_scope("qkv_projection"):
            ...
            if spotedit_active_mask is not None:
                stable_indices = torch.nonzero(~spotedit_active_mask, as_tuple=False).view(-1)
                self._spotedit_stable_k = k.index_select(0, stable_indices).detach().clone()
                self._spotedit_stable_v = v.index_select(0, stable_indices).detach().clone()
                self._spotedit_stable_raw_k = raw_k.index_select(
                    0, stable_indices
                ).detach().clone()
        ...
        if self.openvdn_enabled:
            ...
            with _profile_scope("softmax_output_gate"):
                if active_indices is not None:
                    active_out = out.index_select(0, active_indices)
                    active_out = active_out * self.softmax_gate(
                        x.index_select(0, active_indices)
                    ).to(active_out.dtype)
                    gated = torch.zeros_like(out)
                    gated.index_copy_(0, active_indices, active_out)
                    out = gated
                else:
    ...
    def forward(
        ...
        *,
        ...
        if full_skip:
            active_indices = torch.nonzero(spotedit_active_mask, as_tuple=False).view(-1)
            stable_indices = torch.nonzero(~spotedit_active_mask, as_tuple=False).view(-1)
            active_combined = combined_indices.index_select(0, active_indices)
            with _profile_scope("norm1_and_modulation"):
                h_active = self.norm1(x.index_select(0, active_indices))
                h_active = _modulate_scale_shift(
                    h_active, shift_msa, scale_msa, active_combined, dtype=_BF16_DTYPE
                )
            cached_h = getattr(self, "_spotedit_stable_attention_input", None)
            if cached_h is None or cached_h.shape != (stable_indices.numel(), x.shape[-1]):
                raise RuntimeError(
                    "SpotEdit stable attention-input cache is missing or has the wrong shape"
                )
            h = torch.empty_like(x)
            h.index_copy_(0, active_indices, h_active)
            h.index_copy_(0, stable_indices, cached_h.to(device=x.device, dtype=x.dtype))
        else:
            with _profile_scope("norm1_and_modulation"):
                h = self.norm1(x)
            ...
            if spotedit_active_mask is not None:
                stable_indices = torch.nonzero(~spotedit_active_mask, as_tuple=False).view(-1)
                self._spotedit_stable_attention_input = h.index_select(
                    0, stable_indices
                ).detach().clone()
        h = self.attn(
            h,
        ...
        if full_skip:
            with _profile_scope("attention_residual_gate"):
                active_x = _modulate_gate(
                    residual.index_select(0, active_indices),
                    gate_msa,
                    h.index_select(0, active_indices),
                    active_combined,
                    dtype=_BF16_DTYPE,
                )
            with _profile_scope("norm2_and_modulation"):
                h_active = self.norm2(active_x)
                h_active = _modulate_scale_shift(
                    h_active, shift_mlp, scale_mlp, active_combined, dtype=_BF16_DTYPE
                )
            with _profile_scope("mlp"):
                h_active = self.mlp(h_active)
            with _profile_scope("mlp_residual_gate"):
                active_output = _modulate_gate(
                    active_x, gate_mlp, h_active, active_combined, dtype=_BF16_DTYPE
                )
            cached_output = getattr(self, "_spotedit_stable_output", None)
            if cached_output is None or cached_output.shape != (
                stable_indices.numel(), x.shape[-1]
            ):
            ...
            output.index_copy_(
                0, stable_indices, cached_output.to(device=x.device, dtype=x.dtype)
            )
        else:
            with _profile_scope("attention_residual_gate"):
        ...
        if spotedit_active_mask is not None and spotedit_refresh:
            with _profile_scope("stable_cache_update"):
                stable_indices = torch.nonzero(~spotedit_active_mask, as_tuple=False).view(-1)
                self._spotedit_stable_output = output.index_select(
                    0, stable_indices
                ).detach().clone()
        return output
...
class MiniMaxH3DiTModel(nn.Module):
    ...
    def forward(self, **kwargs: Any) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.arch.openvdn_enabled:
            ...
            if self._source_hybrid_enabled:
...
def _interleaved_openvdn_softmax_attention(
    ...
    dense_q = logical_to_physical.index_select(0, dense_q_logical)
