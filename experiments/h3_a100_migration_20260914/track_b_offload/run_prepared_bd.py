...
def configure(phase):
    ...
    def env(case):
        ...
        value['LD_LIBRARY_PATH'] += ':' + str(ROOT / 'python312_runtime/lib')
...
def run(phase):
    ...
    def capture(pid, owned):
        ...
        if checking:
            ...
            if stage_pids is not None:
                check_clients(scan_clients(unreadable=baseline_unreadable), baseline_clients, owned)
    ...
    def request(server, owned, case, steps, directory, request_manifest):
        ...
        clients = scan_clients(unreadable=baseline_unreadable)
