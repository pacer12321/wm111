"""Bounded D-only four-card diagnostic run. Existing shared card leases, owned cleanup.
...
class Diagnostic(base.ProcessSupervisor):
    def __init__(self):
        selected = replace(profiles.profile('01234567'), code_root=ROOT, master_port=19112,
        ...
        self.case = 'D'
        self.status = {'run_id': self.run_id, 'kind': 'operator_profiling_only', 'cases': {},
    ...
    def launch(self):
        ...
        if self.case == 'D':
            ...
            command = 'source /cache/zhonghao/h3/env_h3_31731.sh && /cache/zhonghao/h3/env/bin/python /cache/zhonghao/h3/profiling_d4_v1/profile_smoke.py'
            smoke = self.spawn(['bash', '-c', command], stdin=subprocess.DEVNULL,
                               stdout=smoke_log, stderr=subprocess.STDOUT)
            self.update('profiler_export_smoke_running', smoke=base.proc_identity(smoke.pid))
        ...
        while time.monotonic() < deadline:
            ...
            try:
                with opener.open('http://127.0.0.1:19112/health', timeout=5) as response:
    ...
    def request(self):
        ...
        base.atomic_json(self.run_dir / 'request.json', {'fields': fields, 'source_video': self.manifest['source_video'],
                         'source_sha256': self.manifest['source_sha256'], 'trace_forward': 3, 'event_forward': 4,
        ...
        command = ['curl', '--silent', '--show-error', '--noproxy', '*', '--connect-timeout', '10',
                   ...
                   '--request', 'POST', 'http://127.0.0.1:19112/v1/videos/sync']
        ...
        for rank in range(4):
            rows = [json.loads(line) for line in (self.run_dir / 'profile' / f'forwards.rank{rank}.jsonl').read_text().splitlines()]
            if [row['forward'] for row in rows] != list(range(1, 50)):
                raise RuntimeError('Unexpected forward count on rank ' + str(rank))
            events = json.loads((self.run_dir / 'profile' / f'event_intervals.rank{rank}.json').read_text())
            names = [r['name'] for r in events['rows']]
            if names.count('linear/T') != 50 or names.count('linear/S') != (50 if self.case == 'D' else 0):
                raise RuntimeError('Actual S/T linear execution count does not match policy')
        traces = list((self.run_dir / 'profile' / 'trace').rglob('trace_view.json'))
        if len(traces) != 1:
            raise RuntimeError('Expected one parsed rank0 operator trace')
        from summarize_events import summarize
        summaries = [summarize(json.loads((self.run_dir / 'profile' / f'event_intervals.rank{rank}.json').read_text()))
                     for rank in range(4)]
        base.atomic_json(self.run_dir / 'interval_summary.json', summaries)
        self.status['cases'][self.case] = {'http_completed': True, 'forward_count_per_rank': 49,
                    'instrumented_request_seconds_NOT_benchmark': elapsed, 'trace': str(traces[0])}
...
if __name__ == '__main__':
    if sys.argv[1:] != ['--run-d-four-cards'] or Path(__file__).resolve().parent != ROOT:
        raise SystemExit('Explicit --run-d-four-cards at the private deployment root required')
