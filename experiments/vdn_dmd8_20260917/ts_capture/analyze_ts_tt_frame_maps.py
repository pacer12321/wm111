...
def branch_maps(payload: dict, device: torch.device) -> dict[str, np.ndarray]:
    ...
    for name, pos_key, coord_key in (
        ("ts", "source_positions", "source_coords"),
        ("tt", "target_positions", "target_coords"),
    ):
...
def plot_grid(grouped: dict, branch: str, output: Path) -> None:
    ...
    for row, layer in enumerate(LAYERS):
        for col, stage in enumerate(STAGES):
            ...
            axes[row, col].set_xlabel("Source key latent-time index" if branch == "ts" else "Target key latent-time index")
            axes[row, col].set_ylabel("Target query latent-time index")
    ...
    title = "T-to-S frame attention" if branch == "ts" else "T-to-T frame attention"
    fig.suptitle(title + " | DMD8 latent-skip, actual executed target queries", fontsize=16)
...
def main() -> None:
    ...
    plot_grid(grouped, "ts", args.output_dir / "ts_early_middle_late_layers8_24_41.png")
    plot_grid(grouped, "tt", args.output_dir / "tt_early_middle_late_layers8_24_41.png")
    (args.output_dir / "attention_metrics.json").write_text(json.dumps({
        "config": {
            ...
            "spatial_query_handling": "up to 16 sampled positions per target latent-time slice; refresh steps use all target queries, skip steps use active target queries only",
            "normalization": "softmax conditional within the plotted key branch",
            "probe": "dense post-RoPE QK reconstruction from current DMD8 latent-skip states using only actually executed target queries",
        },
