#!/usr/bin/env bash
# C only: complete Stage-B weights, strict same-frame visual source; unchanged linear.
...
if [[ "$(hostname)" != "ma-job-042f8bbb-1a1b-41f0-bd5e-6ee86199e9e3-worker-0" ||
  ...
  echo 'Wrong host or missing lease-holding C supervisor' >&2; exit 2
fi
case "${H3_TRIAL_GROUP}" in
  01234567) task_cards=0,1,2,3,4,5,6,7; task_port=19100 ;;
...
task_prefix="/cache/zhonghao/h3/color_trial_v1/results/${H3_TRIAL_GROUP}/${H3_TRIAL_SAMPLE}/C/runs/c_"
if [[ "${ASCEND_RT_VISIBLE_DEVICES:-}" != "${task_cards}" || "${H3_PORT}" != "${task_port}" ||
      ...
      "${H3_GROUP_ID}" != "zhonghao_31731_C_${H3_TRIAL_GROUP}_${H3_MINIMAL_RUN_ID}" ||
  ...
  echo 'Mismatched/noncanonical C group, card, port, output, run or proof' >&2; exit 2
fi
export TMPDIR="/cache/zhonghao/h3/tmp/cmodel_${H3_MINIMAL_RUN_ID}"
...
# Only after that do we put the validated C vendor before the original source.
...
printf 'C sample=%s group=%s cards=%s port=%s run=%s source_policy=strict_same_latent_frame_visual\n' "${H3_TRIAL_SAMPLE}" "${H3_TRIAL_GROUP}" "${task_cards}" "${task_port}" "${H3_MINIMAL_RUN_ID}"
