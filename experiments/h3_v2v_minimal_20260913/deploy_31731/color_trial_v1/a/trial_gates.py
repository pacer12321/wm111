"""Fail-closed gates for the single frozen shirt-color edit on 31731 USP8."""
...
COLOR_PROMPT = ("Change only the man's pale shirt to a solid red shirt. "
...
COLOR_SHA256 = '4fb53ba396d3695eb3bb4eadb0b9fe64dc88592e1c8baf3f6cab59a43f66b0a2'
COLOR_SIZE_BYTES = 1527004
DEFAULT_SAMPLE = 'shirt_red_couple_124'
...
def sample_profile(sample_id=DEFAULT_SAMPLE):
    if sample_id != DEFAULT_SAMPLE:
        raise ValueError('Only the frozen shirt_red_couple_124 sample is allowed')
    directory = ROOT / 'data/shirt_red_couple_124'
    return SampleProfile(sample_id, directory / 'source.mp4', COLOR_PROMPT, directory / 'manifest.json')


def selected_profile(group, sample_id=DEFAULT_SAMPLE):
    sample = sample_profile(sample_id)
    tiny = profiles.profile(group)
    return replace(tiny, output_root=ROOT / 'color_trial_v1/results' / group / sample.sample_id / 'A', code_root=CODE,
...
def source_code_manifest():
    ...
    files.update({f'trial/{name}': CODE / name for name in ('trial_gates.py', 'run_a_trial.py', 'launch_a_trial.sh', 'vlm_gate.py')})
...
def vlm_admission_gate(source, prompt):
    from vlm_gate import verify_admission
    return verify_admission(source, prompt)
...
def sample_gate(sample_id=DEFAULT_SAMPLE, *, transfer=None):
    ...
    if (manifest.get('schema_version') != 1 or manifest.get('sample_id') != DEFAULT_SAMPLE
            or manifest.get('source_video') != str(selected.source)
            or metadata.get('width') != 1280 or metadata.get('height') != 720
            ...
            or router != {'temporal_change_required': False, 'text_only_should_identify_temporal_edit': False,
                          'vlm_necessity_evidence': False}
            or manifest.get('requested_generation') != GENERATION
            or manifest.get('edit_prompt') != selected.prompt):
        raise RuntimeError('Sample manifest does not match the fixed approved temporally preserving shirt-color trial')
    source = file_record(selected.source)
    if (manifest.get('source_sha256') != source['sha256'] or source['sha256'] != COLOR_SHA256
            or source['size_bytes'] != COLOR_SIZE_BYTES):
    ...
    return {'manifest_record': file_record(selected.manifest_path), 'manifest': manifest,
            'source': source, 'probe': probe, 'vlm': vlm_admission_gate(source, selected.prompt)}
