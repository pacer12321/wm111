#!/usr/bin/env bash
# D only: new source self hybrid attention; both target paths preserve old C.
...
: "${D_REVIEWED_POLICY_PATH:?}" "${D_REVIEWED_POLICY_SHA256:?}" "${H3_TRIAL_CASE:?}"
...
if [[ "$(hostname)" != "ma-job-042f8bbb-1a1b-41f0-bd5e-6ee86199e9e3-worker-0" ||
  ...
  echo 'Wrong host or missing lease-holding D supervisor' >&2; exit 2
fi
case "${H3_TRIAL_GROUP}" in
  01234567) task_cards=0,1,2,3,4,5,6,7; task_port=19101 ;;
...
task_prefix="/cache/zhonghao/h3/dualstream_v1/results/${H3_TRIAL_GROUP}/${H3_TRIAL_SAMPLE}/D/runs/d_"
if [[ "${H3_TRIAL_CASE}" != "D" ||
      "${D_REVIEWED_POLICY_PATH}" != "/cache/zhonghao/h3/dualstream_v1/deploy_d/reviewed_policy.json" ||
      ! "${D_REVIEWED_POLICY_SHA256}" =~ ^[0-9a-f]{64}$ ||
      ! -f "${D_REVIEWED_POLICY_PATH}" || -L "${D_REVIEWED_POLICY_PATH}" ||
      ! -f "/cache/zhonghao/h3/dualstream_v1/results/${H3_TRIAL_GROUP}/${H3_TRIAL_SAMPLE}/D/submission_20260914.json" ||
      "${ASCEND_RT_VISIBLE_DEVICES:-}" != "${task_cards}" || "${H3_PORT}" != "${task_port}" ||
      "${H3_ROOT}" != "${task_prefix}"* || "${H3_ROOT##*/}" != *"_${H3_MINIMAL_RUN_ID}" ||
      "$(realpath -e -- "${H3_ROOT}")" != "${H3_ROOT}" || "${H3_OUTPUT}" != "${H3_ROOT}/output" ||
      "${H3_GROUP_ID}" != "zhonghao_31731_D_${H3_TRIAL_GROUP}_${H3_MINIMAL_RUN_ID}" ||
  ...
  echo 'Mismatched/noncanonical D group, card, port, output, run or proof' >&2; exit 2
fi
task_policy_actual="$(sha256sum -- "${D_REVIEWED_POLICY_PATH}")"
if [[ "${task_policy_actual%% *}" != "${D_REVIEWED_POLICY_SHA256}" ]]; then
  echo 'Reviewed D policy SHA changed before launch' >&2; exit 2
fi
export TMPDIR="/cache/zhonghao/h3/tmp/dmodel_${H3_MINIMAL_RUN_ID}"
...
# Only after that do we put the validated D vendor before the original source.
export PYTHONPATH="/cache/zhonghao/h3/dualstream_v1/candidate/vllm-omni${PYTHONPATH:+:${PYTHONPATH}}"
...
printf 'D sample=%s group=%s cards=%s port=%s run=%s policy=D_source_hybrid_same_frame_v1\n' "${H3_TRIAL_SAMPLE}" "${H3_TRIAL_GROUP}" "${task_cards}" "${task_port}" "${H3_MINIMAL_RUN_ID}"
