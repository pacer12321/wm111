...
class MiniMaxH3Attention(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Optional read-only diagnostic.  It captures post-QK-norm, post-RoPE
        # tensors after Ulysses has reconstructed the full sequence.  Normal
        # serving never sets the context, so this has zero effect otherwise.
        self._attention_capture_layer = -1
        self._attention_capture_context: dict[str, Any] | None = None
        self._attention_capture_done = False
    ...
    def _run_openvdn_ulysses(
        ...
        if capture is not None and not self._attention_capture_done:
            requested_layers = capture["layers"]
            if self._attention_capture_layer in requested_layers:
                output_dir = Path(capture["output_dir"])
                output_dir.mkdir(parents=True, exist_ok=True)
                query_indices = capture["query_indices"].to(q_head.device)
                rank_path = output_dir / (
                    f"layer_{self._attention_capture_layer:02d}_rank_{rank}.pt"
                )
                torch.save(
                    {
                        "schema": "h3_openvdn_post_rope_qk_v1",
                        "layer": self._attention_capture_layer,
                        "rank": rank,
                        "world_size": world,
                        "num_local_heads": q_head.shape[2],
                        "head_dim": q_head.shape[3],
                        "softmax_scale": self.softmax_scale,
                        "q_selected": q_head[0].index_select(0, query_indices).detach().cpu(),
                        "k_used": k_head[0, : layout.used_len].detach().cpu(),
                        "query_indices": capture["query_indices"].cpu(),
                        "query_coords": capture["query_coords"].cpu(),
                        "query_active": capture["query_active"].cpu(),
                        ...
                        "target_active_mask": capture["target_active_mask"].cpu(),
                        "spotedit_step": capture["spotedit_step"],
                        "spotedit_refresh": capture["spotedit_refresh"],
                        "spotedit_skip_step": capture["spotedit_skip_step"],
                        "layout": {
                ...
                logger.info(
                    "ATTENTION_QK_CAPTURED layer=%d rank=%d queries=%d used_keys=%d path=%s",
                ...
                self._attention_capture_done = True
        ...
        readout_head = self.linear_attention.forward_head_shard(
            (raw_q[0], raw_k[0], v_head[0]), beta_head[0, :, :, 0], frame_mean, layout,
...
class MiniMaxH3DiTModel(nn.Module):
    ...
    def forward(self, **kwargs: Any) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        capture_layers = {
            int(value)
            for value in os.environ.get("ZHONGHAO_H3_ATTENTION_CAPTURE_LAYERS", "24").split(",")
            if value.strip()
        }
        capture_step = int(os.environ.get("ZHONGHAO_H3_ATTENTION_CAPTURE_STEP", "0"))
        capture_context = None
        if capture_dir and self._spotedit_step == capture_step and openvdn_layout is not None:
            ...
            # img_pos may include alignment-padding visual rows outside the
            # real packed document.  They are conditions by virtue of their
            # false update bit, but they have no valid coordinate/QK row.
            source_positions = source_positions[source_positions < openvdn_layout.used_len]
            target_positions = target_positions[target_positions < openvdn_layout.used_len]
            target_grid = target_positions.view(
                openvdn_layout.num_frames,
                openvdn_layout.frame_height,
                openvdn_layout.frame_width,
            )
            target_active_mask = (
                spot_active_global.index_select(0, target_positions)
                if spot_active_global is not None
                else torch.ones_like(target_positions, dtype=torch.bool)
            )
            if spot_skip_step:
                # At a partial-query step, sample only queries that the model
                # actually evaluates.  Sixteen deterministic, spatially
                # ordered active queries per frame keep the temporal rows
                # balanced while avoiding a misleading map for skipped rows.
                active_grid = target_active_mask.view(
                    openvdn_layout.num_frames,
                    openvdn_layout.frame_height * openvdn_layout.frame_width,
                )
                sampled_per_frame = []
                for frame in range(openvdn_layout.num_frames):
                    active_spatial = torch.nonzero(
                        active_grid[frame], as_tuple=False
                    ).view(-1)
                    if active_spatial.numel() < 16:
                        raise ValueError(
                            f"attention capture needs 16 active queries in frame {frame}, "
                            f"found {active_spatial.numel()}"
                        )
                    pick = torch.linspace(
                        0,
                        active_spatial.numel() - 1,
                        steps=16,
                        device=device,
                    ).round().to(torch.long)
                    sampled_per_frame.append(
                        target_grid[frame].reshape(-1).index_select(
                            0, active_spatial.index_select(0, pick)
                        )
                    )
                sampled = torch.cat(sampled_per_frame)
            else:
                ys = torch.linspace(
                    0,
                    openvdn_layout.frame_height - 1,
                    steps=4,
                    device=device,
                ).round().to(torch.long).unique()
                xs = torch.linspace(
                    0,
                    openvdn_layout.frame_width - 1,
                    steps=4,
                    device=device,
                ).round().to(torch.long).unique()
            ...
            capture_context = {
                ...
                "query_indices": sampled,
                "query_coords": all_img_coords.index_select(0, sampled).detach(),
                "query_active": (
                    spot_active_global.index_select(0, sampled).detach()
                    if spot_active_global is not None
                    else torch.ones_like(sampled, dtype=torch.bool)
                ),
                "source_positions": source_positions.detach(),
                "source_coords": all_img_coords.index_select(0, source_positions).detach(),
                "target_positions": target_positions.detach(),
                "target_coords": all_img_coords.index_select(0, target_positions).detach(),
                "target_active_mask": target_active_mask.detach(),
                "spotedit_step": self._spotedit_step,
                "spotedit_refresh": spot_refresh,
                "spotedit_skip_step": spot_skip_step,
            }
        for block in self.blocks:
