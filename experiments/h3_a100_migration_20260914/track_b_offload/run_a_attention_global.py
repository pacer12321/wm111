"""Run Ref2VA base and aggregate dense temporal attention globally online."""
...
SUPPORT = ROOT / "attention_map_base_dense_support_20260916_v2"
EXPERIMENT = ROOT / "attention_map_base_global_average_20260916_v1"
CANDIDATE = ROOT / "attention_map_base_global_code_20260916_v1/candidate_A_global"
GLOBAL_DIR = EXPERIMENT / "global_attention_sums"
TRANSFORMER_SHA256 = os.environ["ZHONGHAO_H3_BASE_GLOBAL_TRANSFORMER_SHA256"]
PIPELINE_SHA256 = os.environ["ZHONGHAO_H3_BASE_GLOBAL_PIPELINE_SHA256"]
...
STATE = {
    "experiment": "A Ref2VA base dense attention global average",
    "case": "A",
...
def main() -> None:
    ...
    for filename, expected in manifest["cases"]["A"]["model_files"].items():
        ...
        if q.sha(CANDIDATE / q.REL / filename) != candidate_expected:
            raise RuntimeError(f"Isolated A global-attention candidate changed: {filename}")
    ...
    environment["ZHONGHAO_H3_GLOBAL_ATTENTION_DIR"] = str(GLOBAL_DIR)
    environment["ZHONGHAO_H3_GLOBAL_ATTENTION_EXPECTED_STEPS"] = "49"
    environment.pop("ZHONGHAO_H3_ATTENTION_CAPTURE_DIR", None)
    environment["TMPDIR"] = str(ROOT / "tmp/base_dense_attention_global")
    ...
    (EXPERIMENT / "launch.json").write_text(json.dumps({
        ...
        "aggregation": "56 heads x 50 layers x 49 DiT forwards",
        "spatial_query_sampling": "16 target queries per frame",
    ...
    try:
        ...
        with (EXPERIMENT / "server.log").open("x") as log:
            ...
            result.update(
                ...
                aggregation="56 heads x 50 layers x 49 DiT forwards",
                component_maps_expected=50 * 2 * 49,
                formal_speed_comparison=False,
            )
            files = sorted(GLOBAL_DIR.glob("layer_*_rank_*.pt"))
            if len(files) != 100:
                raise RuntimeError(
                    f"Expected 100 layer/rank global-attention sums, found {len(files)}"
                )
