"""Create an explicitly labeled, silent synchronized source/A review derivative."""
...
TARGET = HERE.parent / 'a_50step.mp4'
OUTPUT = HERE / 'source_vs_A_synchronized.mp4'
RECORD = HERE / 'side_by_side_record.json'
EXPECTED = {
    SOURCE: '4fb53ba396d3695eb3bb4eadb0b9fe64dc88592e1c8baf3f6cab59a43f66b0a2',
    TARGET: '392fd304bab4ca92f8a883f6c85c8a27f7adbd61dd5f1b5a6f678e1dee06d9ba',
}
...
def main():
    ...
    with av.open(str(SOURCE)) as left, av.open(str(TARGET)) as right, av.open(str(OUTPUT), 'w') as output:
        ...
        for i, frames in enumerate(zip(left.decode(video=0), right.decode(video=0), strict=True)):
            ...
            draw.text((682, 12), f'A (dense) | shirt -> red | {i / 24:.3f}s', fill='white', font=font)
    ...
    with av.open(str(OUTPUT)) as check:
        ...
        for i, frame in enumerate(check.decode(video=0)):
            ...
            verified += 1
            if i == 60:
                with (HERE / 'source_vs_A_frame_060.png').open('xb') as stream:
    ...
    record = {'output': str(OUTPUT), 'sha256': digest(OUTPUT), 'bytes': OUTPUT.stat().st_size,
              'left_source_sha256': EXPECTED[SOURCE], 'right_A_sha256': EXPECTED[TARGET],
              ...
              'silent': True, 'audio_note': 'Source has no audio; generated A audio deliberately omitted from visual comparison only',
