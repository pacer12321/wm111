...
class LoRA:
    ...
    a: TensorRef
    b: TensorRef
    # First source row in B.  DMD's diffusers SwiGLU stores [value; gate]
    # while the native H3 target stores [gate; value], so the two halves are
    # represented as two target intervals that share A and address different
    # row ranges in B without materialising a reordered full tensor.
    b_row_offset: int = 0
...
def _validate_plan(plan):
    ...
    for lora in plan.loras:
        if len(shape) != 2 or not 0 <= lora.start_row < lora.end_row <= shape[0]:
            raise ValueError("Invalid LoRA row range")
        a_shape, b_shape = lora.a.info["shape"], lora.b.info["shape"]
        row_count = lora.end_row - lora.start_row
        if (len(a_shape) != 2 or a_shape[1] != shape[1] or len(b_shape) != 2
                or b_shape[1] != a_shape[0] or lora.b_row_offset < 0
                or lora.b_row_offset + row_count > b_shape[0]):
            raise ValueError("LoRA shape mismatch")
        if info["dtype"] != "BF16" or lora.a.info["dtype"] not in ("BF16", "F32") or lora.b.info["dtype"] not in ("BF16", "F32"):
            raise ValueError("LoRA merge requires BF16 destination and BF16/F32 factors")
        ...
        if plan.loras:
            ...
            for lora in plan.loras:
                ...
                for row in range(first_row, last_row + 1):
                    ...
                    source_b_row = lora.b_row_offset + b_row
                    b = as_fp32(lora.b, source_b_row * rank, (source_b_row + 1) * rank)
