...
from .openvdn_checkpoint import (
    configure_openvdn_from_env,
    load_complete_openvdn_checkpoint,
    openvdn_cpu_loading_scope,
    strict_base_weights,
)
...
class MiniMaxH3Pipeline(
    ...
    def __init__(
        ...
        *,
        ...
        # This experiment is opt-in.  A keeps its original configuration,
        # weight-loading path and model parameters when the env flag is absent.
        transformer_config, self._openvdn_checkpoint = configure_openvdn_from_env(
            od_config, partition=self.partition
        )
    ...
    def load_weights(
        ...
        if self._openvdn_checkpoint is None:
            return self._load_weights_impl(weights)
        with openvdn_cpu_loading_scope() as loading_record:
            self._openvdn_cpu_loading_record = loading_record
            return self._load_weights_impl(weights, phase_timings=loading_record["phase_seconds"])
    ...
    def _load_weights_impl(
        ...
        *,
        ...
        if self._openvdn_checkpoint is None:
            ...
            # Base QKV is head-interleaved on disk and reordered by the model's
            # existing loader.  Merge the separate Q/K/V LoRAs only AFTER that.
            base_started = time.monotonic()
            loaded = self.transformer.load_weights(
                strict_base_weights(self.transformer, weights, prefix=prefix)
            )
            phase_timings["base"] = time.monotonic() - base_started
            logger.info("OPENVDN_B_PHASE_COMPLETE phase=base seconds=%.3f", phase_timings["base"])
            branch_loaded, self._openvdn_load_record = load_complete_openvdn_checkpoint(
                self.transformer, self._openvdn_checkpoint, phase_timings=phase_timings
            )
            loaded.update(branch_loaded)
    ...
    def _initial_noise(
        ...
        *,
        ...
        latent_w: int,
        audio_t: int,
        noise_reference_latent_h: int | None = None,
        noise_reference_latent_w: int | None = None,
        noise_spatial_stride: int = 1,
        noise_audit: bool = False,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        if (noise_reference_latent_h is None) != (noise_reference_latent_w is None):
            raise ValueError(
                "noise_reference_latent_h and noise_reference_latent_w must be provided together"
            )
        noise_spatial_stride = int(noise_spatial_stride)
        if noise_spatial_stride <= 0:
            raise ValueError("noise_spatial_stride must be positive")
        source_h = int(noise_reference_latent_h or latent_h)
        source_w = int(noise_reference_latent_w or latent_w)
        sampled_h = (source_h + noise_spatial_stride - 1) // noise_spatial_stride
        sampled_w = (source_w + noise_spatial_stride - 1) // noise_spatial_stride
        if (sampled_h, sampled_w) != (latent_h, latent_w):
            raise ValueError(
                "spatially sampled noise shape does not match the requested latent shape: "
                f"source=({source_h}, {source_w}), stride={noise_spatial_stride}, "
                f"sampled=({sampled_h}, {sampled_w}), target=({latent_h}, {latent_w})"
            )
        video_generator = torch.Generator(device="cpu").manual_seed(seed)
        source_video = torch.randn(
            1,
            24,
            latent_t,
            source_h,
        ...
        video = source_video[
            ...,
            ::noise_spatial_stride,
            ::noise_spatial_stride,
        ].contiguous()
        if noise_audit:
            _, rank, _ = _dit_rank_world()
            if rank == 0:
                source_sha256 = hashlib.sha256(source_video.numpy().tobytes()).hexdigest()
                sampled_sha256 = hashlib.sha256(video.numpy().tobytes()).hexdigest()
                logger.info(
                    "MiniMax H3 initial-noise audit: seed=%d source_shape=%s "
                    "stride=%d sampled_shape=%s source_sha256=%s sampled_sha256=%s",
                    seed,
    ...
    def diffuse(
        ...
        *,
        ...
        initial_video, initial_audio = self._initial_noise(
            ...
            latent_w=latent_w,
            audio_t=audio_t,
            noise_reference_latent_h=noise_reference_latent_h,
            noise_reference_latent_w=noise_reference_latent_w,
            noise_spatial_stride=noise_spatial_stride,
        ...
        if capture_dir:
            _, capture_rank, _ = _dit_rank_world()
            if capture_rank == 0:
                if visual_condition is None:
                    raise ValueError("selector capture requires a source-video condition")
                capture_shapes = visual_condition_shapes
                if capture_shapes is None and visual_condition_shape is not None:
                    capture_shapes = [visual_condition_shape]
                if capture_shapes is None or len(capture_shapes) != 1:
                    raise ValueError(
                        "selector capture currently requires exactly one source video; "
                        f"got shapes={capture_shapes!r}"
                    )
                source_t, source_h, source_w = capture_shapes[0]
                source_latent = minimax_h3_unpatchify_video_tokens(
                    visual_condition,
                    latent_shape=(
                        source_t,
                        source_h // 2,
                ...
                temporary = capture_path / "first_x0_and_source_latents.pt.tmp"
                torch.save(
                    {
                        "schema": "h3_selector_capture_v1",
                        "target_first_x0_normalized_latent": video_latent.detach().to(
                            device="cpu", dtype=torch.float16
                        ),
                        "source_clean_normalized_latent": source_latent.detach().to(
                            device="cpu", dtype=torch.float16
                        ),
                        "target_latent_shape": tuple(video_latent.shape),
                        "source_latent_shape": tuple(source_latent.shape),
                        "source_condition_shape": tuple(capture_shapes[0]),
                        "num_schedule_points": int(num_steps),
                        "capture_semantics": (
                            "target is the first DiT x0 prediction; source is the clean "
                            "normalized source-video VAE latent before condition noise"
                        ),
                    },
                    temporary,
                )
                temporary.replace(capture_path / "first_x0_and_source_latents.pt")
    ...
    def forward(self, request: DiffusionRequestBatch) -> DiffusionOutput:
        ...
        noise_reference_latent_h = extra.get("noise_reference_latent_h")
        noise_reference_latent_w = extra.get("noise_reference_latent_w")
        if noise_reference_latent_h is not None:
            noise_reference_latent_h = int(noise_reference_latent_h)
        if noise_reference_latent_w is not None:
            noise_reference_latent_w = int(noise_reference_latent_w)
        noise_spatial_stride = int(extra.get("noise_spatial_stride", 1))
        noise_audit = bool(extra.get("noise_audit", False))
        if task != "t2va" and (
            noise_reference_latent_h is not None
            or noise_reference_latent_w is not None
            or noise_spatial_stride != 1
        ):
            raise ValueError("controlled initial-noise sampling is supported only for t2va")
