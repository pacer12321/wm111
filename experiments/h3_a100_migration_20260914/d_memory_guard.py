"""Low-cost cgroup v1 guard; only the controller handles stopping its own server."""
import json
import threading
import time
...
def pressure_reason(sample, initial_oom):
    ...
    if usage > MAX_USAGE_BYTES:
        return (f'proactive memory stop: usage={usage}, threshold={MAX_USAGE_BYTES}, '
