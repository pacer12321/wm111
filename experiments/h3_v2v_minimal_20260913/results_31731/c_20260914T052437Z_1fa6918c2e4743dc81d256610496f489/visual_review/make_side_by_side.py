"""Create an explicitly labeled, silent synchronized source/C review derivative."""
...
SOURCE = HERE.parent.parent / 'a_20260913T153514Z_91c6c0f98ba84c7c828202d5934176cd/source.mp4'
TARGET = HERE.parent / 'c_50step.mp4'
OUTPUT = HERE / 'source_vs_C_synchronized.mp4'
RECORD = HERE / 'side_by_side_record.json'
EXPECTED = {
    SOURCE: '4fb53ba396d3695eb3bb4eadb0b9fe64dc88592e1c8baf3f6cab59a43f66b0a2',
    TARGET: 'f17aedffb0f1492266e81e6d866a9fb887ac653706291d7f6fb3c5dbb313b593',
}
...
def main():
    ...
    with av.open(str(SOURCE)) as left, av.open(str(TARGET)) as right, av.open(str(OUTPUT), 'w') as output:
        ...
        for i, frames in enumerate(zip(left.decode(video=0), right.decode(video=0), strict=True)):
            ...
            draw.text((682, 12), f'C (VDN+same-frame) | shirt -> red | {i / 24:.3f}s', fill='white', font=font)
    ...
    with av.open(str(OUTPUT)) as check:
        ...
        for i, frame in enumerate(check.decode(video=0)):
            ...
            verified += 1
            if i == 60:
                with (HERE / 'source_vs_C_frame_060.png').open('xb') as stream:
    ...
    record = {'output': str(OUTPUT), 'sha256': digest(OUTPUT), 'bytes': OUTPUT.stat().st_size,
              'left_source_sha256': EXPECTED[SOURCE], 'right_C_sha256': EXPECTED[TARGET],
              ...
              'silent': True, 'audio_note': 'Source has no audio; generated C audio deliberately omitted from visual comparison only',
