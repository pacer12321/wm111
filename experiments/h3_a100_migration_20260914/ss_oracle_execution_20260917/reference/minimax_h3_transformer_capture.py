                ...
                torch.save(
                    {
                        ...
                        "query_stream": capture["query_stream"],
...
class MiniMaxH3DiTModel(nn.Module):
    ...
    def forward(self, **kwargs: Any) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if capture_dir and self._attention_capture_step in capture_steps:
            ...
            query_stream = os.environ.get(
                "ZHONGHAO_H3_ATTENTION_CAPTURE_QUERY_STREAM", "target"
            ).strip().lower()
            if query_stream not in {"source", "target"}:
                raise ValueError(
                    "ZHONGHAO_H3_ATTENTION_CAPTURE_QUERY_STREAM must be "
                    f"'source' or 'target', got {query_stream!r}"
                )
            query_grid = (
                source_positions if query_stream == "source" else target_positions
            ).view(frames, height, width)
            ...
            sampled = query_grid[:, yy.reshape(-1), xx.reshape(-1)].reshape(-1)
            capture_context = {
