#!/usr/bin/env python3
"""31731 fixed USP8 C: frozen lake (default) or reverse, smoke/formal/cleanup.

Opt-in only. Imports independent reviewed validation safety helpers, never
mutates their globals. Strict same-latent-frame visual source only: no training, retries or choices
...
class CTrialSupervisor(base.ProcessSupervisor):
    def __init__(self, group, sample_id=gates.DEFAULT_SAMPLE):
        ...
        self._request_order = []
        self.status = {'case': 'C', 'sample_id': self.sample.sample_id, 'group': group,
                       ...
                       'attention_policy': 'C strict same-latent-frame visual source in target softmax queries; B target neighborhoods and linear unchanged',
    ...
    def update(self, phase, **values):
        ...
        if self.run_dir is not None:
            base.atomic_json(self.run_dir / 'c_status.json', self.status)
            base.atomic_json(self.selected.output_root / 'c_status.json', self.status)
    ...
    def acquire(self):
        ...
        new_run = self.selected.output_root / 'runs' / f'c_{stamp}_{self.run_id}'
        profiles.canonical_private(new_run)
        new_run.mkdir(parents=True, exist_ok=False)
        self.run_dir = new_run
        for path in (self.run_dir / 'output', gates.ROOT / 'tmp' / f'cmodel_{self.run_id}'):
            profiles.canonical_private(path)
    ...
    def preflight(self):
        ...
        self.update('preflight_passed', resource_snapshot=idle, card_health_memory=health, host_memory=memory,
                    ...
                    note='C strict visual-source same latent frame; own C tiny plus fresh smoke/load/PID/strict metadata gates.')
    ...
    def env(self):
        ...
        env.update(H3_ROOT=str(self.run_dir), H3_OUTPUT=str(self.run_dir / 'output'), H3_MINIMAL_RUN_ID=self.run_id,
                   ...
                   H3_PORT=str(self.selected.master_port), H3_GROUP_ID=f'zhonghao_31731_C_{self.selected.group}_{self.run_id}',
    ...
    def launch(self):
        ...
        self.server = self.spawn(['bash', str(self.selected.code_root / 'launch_c_trial.sh')],
        ...
        while time.monotonic() < deadline:
            if self.server.poll() is not None:
                raise RuntimeError(f'C server exited during startup: {self.server.returncode}')
            try:
                ...
                pass
            time.sleep(2)
        raise TimeoutError('C server did not become healthy within 2700 seconds')

    def request(self, name, steps):
        expected = [('smoke_2step', 2), ('c_50step', 50)]
        ...
        if steps == 50 and not self.status.get('formal_smoke_gate_passed'):
            raise RuntimeError('C formal request cannot bypass the fresh same-service smoke gate')
        ...
        while child.poll() is None:
            if self.server.poll() is not None:
                raise RuntimeError('C server exited during request')
    ...
    def smoke_gate(self):
        ...
        if (not set(workers.values()).issubset(owned)
            ...
            raise RuntimeError('Selected-card workers are not all owned by this same C service')
        ...
        strict = gates.parse_strict_metadata(log_raw.decode('utf-8', errors='replace'), set(workers.values()), self.sample.sample_id)
    ...
    def run_requests(self):
        ...
        result = self.request('c_50step', 50)
        ...
        if hashlib.sha256(raw[:prefix_size]).hexdigest() != smoke_proof['server_log_prefix_sha256']:
            raise RuntimeError('C server log prefix changed after the smoke gate')
        workers = {int(pid) for pid in smoke_proof['loaded_records_by_pid']}
        strict = gates.parse_strict_metadata(raw.decode('utf-8', errors='replace'), workers, self.sample.sample_id, requests=2)
        base.atomic_json(self.run_dir / 'formal_strict_metadata.json', strict)
...
def main(argv=None):
    ...
    owner = CTrialSupervisor(args.group, args.sample)
    ...
    try:
        ...
        try:
            ...
            if not clean and error is None:
                error = 'Owned C processes remain'
            ...
            owner.update('needs_attention' if owner.status.get('needs_attention') else ('failed' if error else 'formal_completed_review_required'),
                         error=error, finished_at=base.utc_now(), note='C strict visual-source same latent frame; B weights/linear unchanged. Edit quality, NFE and acceleration remain unreviewed.')
        finally:
            owner.release_locks()
