...
class MiniMaxH3Attention(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Optional read-only diagnostic state.  Captures local sequence shards
        # of post-QK-norm, post-RoPE Q/K without changing the dense attention
        # calculation or its backend.
        ...
        # Optional read-only global diagnostic.  Each Ulysses rank owns half
        # of the heads after pre_attention, so it can accumulate an exact
        # source-conditioned temporal map without materializing full Q/K or
        # communicating diagnostic tensors.  Small per-layer sums are written
        # only after the final requested denoising forward.
        self._global_attention_context: dict[str, Any] | None = None
        self._global_attention_sum: torch.Tensor | None = None
        self._global_attention_observations = 0
    ...
    def _run_packed_attention(
        ...
        *,
        ...
        global_context = self._global_attention_context
        ...
        # Mirror Attention._forward_impl exactly, inserting only a read-only
        # statistic between Ulysses pre_attention and the original kernel.
        strategy = self.attention._get_active_parallel_strategy()
        if strategy.name != "ulysses" or self.attention.use_ring:
            raise ValueError("Global Ref2VA attention averaging requires pure Ulysses")
        q_head, k_head, v_head, head_metadata, strategy_context = strategy.pre_attention(
            q.unsqueeze(0), k.unsqueeze(0), v.unsqueeze(0), metadata
        )
        self._accumulate_global_temporal_attention(
            q_head[0], k_head[0], global_context
        )
        head_metadata = self.attention._with_kv_cache_dtype(head_metadata)
        out = self.attention._run_local_attention(
            q_head, k_head, v_head, head_metadata
        )
        return strategy.post_attention(out, strategy_context).squeeze(0)

    @torch.compiler.disable
    def _accumulate_global_temporal_attention(
        self,
        q_head: torch.Tensor,
        k_head: torch.Tensor,
        context: dict[str, Any],
    ) -> None:
        """Accumulate one layer/timestep map, averaged over local heads/Q.

        q_head/k_head are post-QK-norm, post-RoPE, full-sequence Ulysses
        tensors with only this rank's head shard.  For each source frame we
        compute logsumexp over all spatial keys.  A softmax over those 37
        frame log-masses is exactly the original dense attention conditioned
        on the key being a source-video token; the full-softmax denominator
        cancels under this conditioning.
        """
        step = int(context["step"])
        expected_steps = int(context["expected_steps"])
        if step != self._global_attention_observations:
            raise ValueError(
                "Global attention observation order mismatch: "
                f"step={step} observed={self._global_attention_observations}"
            )
        if q_head.ndim != 3 or k_head.ndim != 3:
            raise ValueError("Expected Ulysses q/k as [sequence, local_heads, head_dim]")
        query_indices = context["query_indices"].to(q_head.device)
        source_positions = context["source_positions"].to(k_head.device)
        source_coords = context["source_coords"].to(k_head.device)
        frames = int(context["frames"])
        samples_per_frame = int(context["samples_per_frame"])
        if query_indices.numel() != frames * samples_per_frame:
            raise ValueError("Global attention query grid changed")
        selected_q = q_head.index_select(0, query_indices)
        source_times = torch.unique(source_coords[:, 0], sorted=True)
        if source_times.numel() != frames:
            raise ValueError("Global attention source-frame count changed")

        frame_log_mass = []
        for source_time in source_times:
            frame_mask = source_coords[:, 0] == source_time
            frame_positions = source_positions[frame_mask]
            frame_k = k_head.index_select(0, frame_positions)
            logits = torch.einsum(
                "qhd,khd->hqk", selected_q, frame_k
            ) * self.softmax_scale
            frame_log_mass.append(torch.logsumexp(logits.float(), dim=-1))
            del frame_k, logits
        # [local_heads, sampled_queries, source_frames]
        temporal = torch.softmax(torch.stack(frame_log_mass, dim=-1), dim=-1)
        temporal = temporal.view(
            temporal.shape[0], frames, samples_per_frame, frames
        ).mean(dim=(0, 2))
        # Keep the tiny 37x37 running sum on GPU so the diagnostic does not
        # introduce one device synchronization per layer and timestep.
        observation = temporal.detach().double()
        if self._global_attention_sum is None:
            self._global_attention_sum = torch.zeros_like(observation)
        self._global_attention_sum.add_(observation)
        self._global_attention_observations += 1

        if self._global_attention_observations == expected_steps:
            ...
            torch.save(
                {
                    "schema": "h3_dense_global_temporal_sum_v1",
                    "attention_mode": "ref2va_base_dense",
                    "layer": self._attention_capture_layer,
                    "rank": rank,
                    "world_size": int(sp_group.ulysses_world_size),
                    "local_heads": int(q_head.shape[1]),
                    "head_dim": int(q_head.shape[2]),
                    "timesteps": expected_steps,
                    "frames": frames,
                    "samples_per_target_frame": samples_per_frame,
                    "temporal_conditional_sum": self._global_attention_sum.cpu(),
                    "definition": (
                        "mean over local heads and sampled spatial target queries; "
                        "source-conditioned dense softmax; summed over timesteps"
                    ),
                },
                output_path,
            )
            logger.info(
                "GLOBAL_DENSE_ATTENTION_SAVED layer=%d rank=%d timesteps=%d path=%s",
        ...
        *,
        ...
        if capture is not None:
            ...
            if (
                ...
                if total * world != global_len:
                    raise ValueError(
                        "Dense attention capture requires equal contiguous SP shards: "
                ...
                torch.save(
                    {
                        "schema": "h3_dense_local_post_rope_qk_v1",
                        "attention_mode": "ref2va_base_dense",
                        "layer": self._attention_capture_layer,
                        "step": step,
                        "rank": rank,
                ...
                logger.info(
                    "DENSE_ATTENTION_QK_CAPTURED step=%d layer=%d rank=%d "
        ...
        if self.openvdn_enabled:
            ...
            if self.openvdn_interior_group_size:
                out = grouped_anchor_softmax_attention(
                    q,
                    k,
                    v,
                    openvdn_layout,
...
class MiniMaxH3DiTModel(nn.Module):
    ...
    def forward(self, **kwargs: Any) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.arch.openvdn_enabled:
            ...
            expected_video_rows = (
                self.arch.openvdn_num_frames
                * self.arch.openvdn_frame_height
                * self.arch.openvdn_frame_width
            )
            if target_video_pos.numel() != expected_video_rows:
                raise ValueError(
                    "OpenVDN target video layout mismatch: "
                    f"{target_video_pos.numel()} rows != {expected_video_rows}"
                )
            video_start = int(target_video_pos[0].item())
            if int(target_video_pos[-1].item()) - video_start + 1 != target_video_pos.numel():
                raise ValueError("OpenVDN requires contiguous target video rows")
            text_pos_dev = text_pos.to(device)
            text_start = int(text_pos_dev[0].item())
            openvdn_layout = OpenVDNLayout(
                used_len=int(cu_seqlens[1].item()),
                video_start=video_start,
                num_frames=self.arch.openvdn_num_frames,
                tokens_per_frame=self.arch.openvdn_frame_height * self.arch.openvdn_frame_width,
                frame_height=self.arch.openvdn_frame_height,
                frame_width=self.arch.openvdn_frame_width,
                text_start=text_start,
                text_len=text_pos_dev.numel(),
            )
        ...
        global_attention_dir = os.environ.get(
            "ZHONGHAO_H3_GLOBAL_ATTENTION_DIR"
        )
        global_expected_steps = int(
            os.environ.get("ZHONGHAO_H3_GLOBAL_ATTENTION_EXPECTED_STEPS", "49")
        )
        if global_expected_steps <= 0:
            raise ValueError("Global attention expected steps must be positive")
        capture_context = None
        global_attention_context = None
        capture_this_step = bool(
            capture_dir and self._attention_capture_step in capture_steps
        )
        if capture_this_step or global_attention_dir:
            used_len = int(cu_seqlens[1].item())
            ...
            if target_positions.numel() != frames * per_frame:
                raise ValueError(
                    "Dense attention capture cannot form target video grid: "
            ...
            if (
                ...
                raise ValueError(
                    "Dense attention capture source/target grids differ: "
            ...
            if global_attention_dir:
                global_attention_context = {
                    "output_dir": global_attention_dir,
                    "step": self._attention_capture_step,
                    "expected_steps": global_expected_steps,
                    "query_indices": sampled.detach(),
                    "source_positions": source_positions.detach(),
                    "source_coords": source_coords.detach(),
                    "frames": frames,
                    "samples_per_frame": int(yy.numel()),
                    "layout": layout,
                }
        for block in self.blocks:
            block.attn._attention_capture_context = capture_context
            block.attn._global_attention_context = global_attention_context
            hidden = block(
                hidden,
                t_emb=t_emb,
            ...
            block.attn._global_attention_context = None
