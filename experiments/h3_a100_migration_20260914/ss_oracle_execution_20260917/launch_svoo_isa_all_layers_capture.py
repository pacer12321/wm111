...
os.environ.update(
    ...
    ZHONGHAO_H3_SS_ORACLE_CAPTURE_LAYERS=",".join(str(i) for i in range(48)),
    ZHONGHAO_H3_EXPERIMENT_LABEL=(
        "Sampled source-Q and full K/V capture for all 48 layers at step 4 on "
        "the frozen fastest pipeline. No attention route or selector is changed; "
        "the capture is for SVOO per-layer budget screening only."
    ),
)
