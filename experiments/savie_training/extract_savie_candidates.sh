...
MEMBERS="$ROOT/manifests/savie_available_content"
OUTPUT="$ROOT/savie_candidates/videos"
LOG_DIR="$ROOT/logs"
mkdir -p "$OUTPUT" "$LOG_DIR"

groups=(source global_freeform1 global_freeform2 global_style1)
for group in "${groups[@]}"; do
  ...
  printf '%s expected=%s found=%s\n' "$group" "$expected" "$found" \
    | tee "$LOG_DIR/extract_savie_${group}.status"
  [[ "$found" -eq "$expected" ]]
done

date -Iseconds > "$LOG_DIR/extract_savie_complete.timestamp"
