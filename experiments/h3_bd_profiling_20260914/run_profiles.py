"""Bounded B then D diagnostic run. Existing shared card leases, owned cleanup.
...
class Diagnostic(base.ProcessSupervisor):
    def __init__(self):
        selected = replace(profiles.profile('01234567'), code_root=ROOT, master_port=19111,
    ...
    def launch(self):
        ...
        if self.case == 'B':
            ...
            command = 'source /cache/zhonghao/h3/env_h3_31731.sh && /cache/zhonghao/h3/env/bin/python /cache/zhonghao/h3/profiling_bd_v1/profile_smoke.py'
        ...
        while time.monotonic() < deadline:
            ...
            try:
                with opener.open('http://127.0.0.1:19111/health', timeout=5) as response:
    ...
    def request(self):
        ...
        command = ['curl', '--silent', '--show-error', '--noproxy', '*', '--connect-timeout', '10',
                   ...
                   '--request', 'POST', 'http://127.0.0.1:19111/v1/videos/sync']
...
if __name__ == '__main__':
    if sys.argv[1:] != ['--run-b-then-d'] or Path(__file__).resolve().parent != ROOT:
        raise SystemExit('Explicit --run-b-then-d at the private deployment root required')
