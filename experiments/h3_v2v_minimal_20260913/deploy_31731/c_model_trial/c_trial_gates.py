...
B_CODE = Path('/cache/zhonghao/h3/b_model_trial_code')
...
B_GATES_SHA256 = 'b0e5b1aafe83b15a9b0fcaa3775f690ec2076c014a9f08842a12f8bb32296507'
...
def selected_profile(group, sample_id=DEFAULT_SAMPLE):
    ...
    return replace(profiles.profile(group), output_root=ROOT / 'c_model_trial' / group / sample.sample_id / 'C',
...
def parse_strict_metadata(log_text, worker_pids, sample_id, *, requests=1):
    ...
    for line in log_text.splitlines():
        ...
        if (row['target_shape'] != (37, 48, 84) or type(row['text_len']) is not int or row['text_len'] <= 0
                ...
                or (sample_id == 'explicit_reverse_couple_124' and row['source_audio_t'] != 0)):
