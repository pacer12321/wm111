"""One authorized 2-step D smoke, same model settings, no formal request."""
import fcntl
import json
import os
import shutil
...
q = d.q
CONTROL = d.ROOT / 'd_smoke_after_cleanup_20260914'
HISTORY = q.WORK / 'history/30674_d_shared_pressure_20260914'
...
def launch():
    ...
    assert old['status'] == 'failed' and old['pid'] == 1469203
    assert old['memory_guard_reason'].startswith('proactive memory stop:')
    assert q.pid_identity(1469203) is None
